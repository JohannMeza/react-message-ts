...
import { fetchMessagingGroup } from '@src/modules/dashboard/dashboard-actions';
import { paddingResponsive } from '../../messaging-styles';
import { BoxSending, BoxUpdate } from './SectionMessageSend';
import { sendMessageToGroup, updateMessageToGroup } from '../../messaging-actions';
...
export const MessagingGroup: FC<PropsWithChildren> = () => {
  const groupData = useEvent(groupDataEvent);
  const [messages] = useFetchAction(fetchMessagingGroup, [groupData?.idGroupMember]);
  ...
  const [callMessages] = useCallAction(fetchMessagingGroup, {
    onCompleted() {
      downScroll();
    }
  });
  ...
  const [callSendMessage] = useCallAction(sendMessageToGroup, {
    onCompleted() {
      callMessages(groupData?.idGroupMember);
    }
  });

  const [dataLabel, setDataLabel] = useState({ active: false, message: '' });
  const [callUpdateMessage] = useCallAction(updateMessageToGroup, {
    onCompleted() {
      callMessages(groupData?.idGroupMember);
      ...
      >
          ...
          <Stack flexDirection="row" alignItems="center" gap={2}>
            <Stack flexDirection="row" alignItems="center" gap={1}>
              <IconButton
                size="small"
                color="default"
                onClick={() => callMessages(groupData.idGroupMember)}
                sx={{ display: { xs: 'block', md: 'none' } }}
              >
                <KeyboardArrowLeftIcon />
              </IconButton>
              <Avatar src={groupData.pathImage} />
            </Stack>
            <Box>
              <Typography
                sx={{ cursor: 'pointer' }}
...

        {
          ...
          : <BoxSending 
              ...
              stateContact={groupData.stateContact}
            />
        }
