...
import { Avatar, Box, Grid, IconButton, Stack, Typography, useTheme, Switch  } from '@mui/material';
  ...
  return (
    ...
    >
      <Box paddingX={1} width={1}>
        <IconButton 
          color='default'
          onClick={() => handleChangeActiveProfileUser(ActiveProfileUserEnum.NONE)}
        >
          <CloseIcon />
        </IconButton>
      </Box>
      ...
      <Stack width={1} height={1} overflow="auto" gap={2}>
        <Stack paddingX={3} width={1} gap={2}>
          <Stack alignItems="center" width={1}>
            ...
            <Typography fontSize={20} fontWeight={700}>Esther Howard</Typography>
          </Stack>

          <Box>
            <Typography fontSize={16} fontWeight={500}>Información</Typography>
          ...
          <Box sx={{ textWrap: 'nowrap' }}>
            <Typography 
              ...
              fontWeight={500}
              sx={{ cursor: 'pointer' }}
              onClick={() => handleChangeProfileUser(ProfileUserCurrentView.FILE, false)}
            >
              Archivos, enlaces y documentos
            </Typography>
            <Grid container spacing={1}>
              <Grid item xs={4}>
                <Box bgcolor="primary.main" height="80px" flexShrink={1} padding={1} borderRadius="5px"
                  sx={{ cursor: 'pointer' }}
                  onClick={() => handleChangeProfileUser(ProfileUserCurrentView.FILE, false)}
                >
                  <img 
        ...
        <Box width={1}>
          <Typography fontSize={16} fontWeight={500} paddingX={3}>2 grupos en común</Typography>

          {
            chatsMockup.map((el, index) => (
              <Stack 
                sx={{ '&:hover': { backgroundColor: 'background_colors_opacity.60' }, cursor: 'pointer' }} 
                flexDirection="row" 
                alignItems="center"
                paddingX={3} 
                paddingY={1}
        ...
        <Box width={1}>
          <ListSettingBox onClick={() => handleChangeProfileUser(ProfileUserCurrentView.DIRECT_MESSAGE, false)}>
            <Stack flexDirection="row" gap={2}>
              <StarIcon color="action" />
              <Typography fontSize={16} fontWeight={500} color="grey.700">Mensajes destacados</Typography>
            </Stack>
            <ChevronRightIcon />
          </ListSettingBox>
          <ListSettingBox>
            <Stack flexDirection="row" gap={2}>
              ...
              <Typography fontSize={16} fontWeight={500} color="grey.700">Silenciar notificaciones</Typography>
            </Stack>
          ...
          <ListSettingBox>
            <Stack flexDirection="row" gap={2}>
              ...
              <Typography fontSize={16} fontWeight={500} color="red.main">Bloquear a Esther Howard</Typography>
            </Stack>
          </ListSettingBox>
          <ListSettingBox>
            <Stack flexDirection="row" gap={2}>
              <ThumbDownIcon color="error" />
              <Typography fontSize={16} fontWeight={500} color="red.main">Reportar a Esther Howard</Typography>
            </Stack>
          </ListSettingBox>
          <ListSettingBox>
            <Stack flexDirection="row" gap={2}>
              <DeleteIcon color="error" />
              <Typography fontSize={16} fontWeight={500} color="red.main">Eliminar chat</Typography>
            </Stack>
          </ListSettingBox>
        </Box>
      </Stack>
