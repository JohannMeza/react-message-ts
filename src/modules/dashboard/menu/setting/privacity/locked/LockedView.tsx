...
export const LockedView: FC<PropsWithChildren> = () => {
  ...
  return (
    <Box display="grid" gridTemplateRows="auto auto auto" height={1} alignContent="flex-start">
      <ModalContactsLockedView open={open} setOpen={setOpen} contacts={contactsMockup} />
      <Stack flexDirection="row" alignItems="center" gap={2} padding="10px" height="auto">
        <IconButton 
        ...
        <Typography fontSize={20} fontWeight={700} color="grey.700">Contactos Bloqueados</Typography>
      </Stack>
      <ListSettingBox onClick={() => setOpen(true)}>
        <Stack flexDirection="row" gap={2}>
          <PersonAddIcon color="action" />
          <Typography fontSize={16} fontWeight={500} color="grey.700">Bloquear un contacto</Typography>
        </Stack>
      </ListSettingBox>
          ...
          contactsBloquedMockup.map((el, index) => (
            <ListSettingBox key={index}>
              <Stack flexDirection="row" gap={2} alignItems="center">
                ...
                <Typography fontSize={16} fontWeight={500} color="grey.700">{el.name}</Typography>
              </Stack>
              <IconButton color='default'><CloseIcon /></IconButton>
            </ListSettingBox>
          ))
        }
      </Stack>
