...
export const ChatItemView: FC<ChatItemType> = ({ message, name, avatar, state, id, createdAt }) => {
    ...
    >
      ...
      <Box width={1}>
        <Stack flexDirection="row" justifyContent="space-between" alignItems="center" width={1}>
          <Typography fontSize={16} fontWeight={700} color="grey.700">{ name }</Typography>
          <Typography fontSize={12} color="grey.700">{ getPastDate(createdAt) }</Typography>
        </Stack>
        <Stack flexDirection="row" gap={1}>
          { MessageStateIcon[state] }
          <Typography fontSize={14} fontWeight={400} color="gray.600">{ message }</Typography>
        </Stack>
      </Box>
    </Stack>
  );
